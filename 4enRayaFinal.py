...
def ganador(tablero, jugador):
    ...
    for i in range(columnas):
        ...
        if winner==True:
            return winner
    diagonales1 = [''.join([tablero[i, d+i] if (d+i < columnas and d+i>=0) else ' ' for i in range(filas) ]) for d in range(-filas, columnas)]
    diagonales2 = [''.join([tablero[i, d-i] if (d-i < columnas and d-i>=0) else ' ' for i in range(filas) ]) for d in range(columnas+filas)]        

    for diagonal in diagonales1+diagonales2:  #crea una lista con las 2 diagonales
        winner=4*jugador in diagonal 
        if winner==True:
           return winner 
    return winner
...
SQUARE_SIZE = 80
WIDTH = columnas*SQUARE_SIZE
HEIGHT = filas*SQUARE_SIZE
ROWS = filas
COLS = columnas
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            if columnaJugador<columnas and casillasLibres[columnaJugador]>0:   
                ...
                if winner2==True:
                    ...
                    win_text = font.render("You Win!", True, BLANCO)
